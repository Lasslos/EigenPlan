...
GridEntryPositionItem? _subjectSlot(GridEntry entry) => entry.positionOfType('SUBJECT') ?? entry.positionOfType('INFO');
...
GridEntryPositionItem? _teacherSlot(GridEntry entry) => entry.positionOfType('TEACHER') ?? entry.positionOfType('CLASS');

GridEntryPositionItem? _roomSlot(GridEntry entry) => entry.positionOfType('ROOM');

String _text(GridEntryPositionElement? element, {required bool short}) {
  if (element == null) {
    return '';
  }
  return (short ? element.shortName : element.longName) ?? element.displayName ?? '';
}
...
class GridEntryWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    var userData = ref.watch(selectedUntisSessionProvider.select((value) => (value as ActiveUntisSession).userData));
    var resolvedSubject = entry.resolveSubject(userData.subjects);
    ...
    if (entry.isExam) {
      ...
      statusColor = ref.watch(customSubjectColorsProvider)[resolvedSubject?.key] ?? CustomSubjectColor.regularColor;
    } else {
    ...
    Color textColor = entry.isCancelled ? Theme.of(context).textTheme.bodyLarge!.color! : statusColor.textColor;

    return Card(
      margin: const EdgeInsets.all(2),
      color: entry.isCancelled ? Theme.of(context).cardColor : statusColor.color,
      ...
      child: InkWell(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            bool useShort = constraints.maxWidth < 100 ||
            ...
            Widget lineOf(String orgText, String text) => RichText(
                  text: TextSpan(
                    children: [
                      TextSpan(
                        text: orgText,
                        style: const TextStyle(color: Colors.white60, decoration: TextDecoration.lineThrough),
                      ),
                      if (orgText.isNotEmpty && text.isNotEmpty) const TextSpan(text: ' '),
                      TextSpan(text: text),
                    ],
            ...
            return Column(
              ...
              children: [
                if (subjectText.isNotEmpty || orgSubjectText.isNotEmpty) Flexible(child: lineOf(orgSubjectText, subjectText)),
                if (teacherText.isNotEmpty || orgTeacherText.isNotEmpty) Flexible(child: lineOf(orgTeacherText, teacherText)),
                if (roomText.isNotEmpty || orgRoomText.isNotEmpty) Flexible(child: lineOf(orgRoomText, roomText)),
              ],
            );
          },
        ),
...
class GridEntryDetailsView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    var userData = ref.watch(selectedUntisSessionProvider.select((value) => (value as ActiveUntisSession).userData));
    var resolvedSubject = entry.resolveSubject(userData.subjects);
    var subjectId = resolvedSubject?.key;
    var customSubjectColor = ref.watch(customSubjectColorsProvider)[subjectId] ?? CustomSubjectColor.regularColor;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          'Stundendetails',
          style: TextStyle(color: statusColor?.textColor ?? customSubjectColor.textColor),
        ),
      ),
      body: ListView(
        children: [
          ...
          SizedBox(
            ...
            child: Column(
              ...
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                Icon(Icons.book_outlined, size: 50, color: statusColor?.color ?? customSubjectColor.color),
                ListTile(
                  title: Text.rich(
                    TextSpan(
                      children: [
                        if (orgHeadline.isNotEmpty && orgHeadline != headline)
                          TextSpan(
                            text: orgHeadline,
                            style: TextStyle(color: Theme.of(context).disabledColor, decoration: TextDecoration.lineThrough),
                          ),
                        if (orgHeadline.isNotEmpty && orgHeadline != headline) const TextSpan(text: ' '),
          ...
          if (subjectSlot?.current.shortName != null && subjectSlot!.current.shortName != subjectSlot.current.longName)
            ListTile(
              ...
              TextSpan(
                children: [
                  if (teacherSlot?.removed != null)
                    TextSpan(
                      text: _text(teacherSlot!.removed, short: false).let((s) => s.isEmpty ? 'Kein Lehrer' : s),
                      style: TextStyle(color: Theme.of(context).disabledColor, decoration: TextDecoration.lineThrough),
                    ),
                  if (teacherSlot?.removed != null) const TextSpan(text: ' '),
                  TextSpan(text: _text(teacherSlot?.current, short: false).let((s) => s.isEmpty ? 'Kein Lehrer' : s)),
                ],
              ),
            ),
          ),
              ...
              TextSpan(
                children: [
                  if (roomSlot?.removed != null)
                    TextSpan(
                      text: _text(roomSlot!.removed, short: false).let((s) => s.isEmpty ? 'Kein Raum' : s),
                      style: TextStyle(color: Theme.of(context).disabledColor, decoration: TextDecoration.lineThrough),
                    ),
                  if (roomSlot?.removed != null) const TextSpan(text: ' '),
                  TextSpan(text: _text(roomSlot?.current, short: false).let((s) => s.isEmpty ? 'Kein Raum' : s)),
                ],
              ),
            ),
          ),
              ...
              leading: SizedBox(
                ...
                child: Center(
                  child: Container(
                    width: 20,
                    height: 20,
                    decoration: BoxDecoration(color: customSubjectColor.color, borderRadius: BorderRadius.circular(6)),
                  ),
                ),
              ),
                ...
                showDialog(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content: MaterialColorPicker(
                      onColorChange: (color) {
                        ...
                        ref.read(customSubjectColorsProvider.notifier).add(
                              CustomSubjectColor(subjectId, color, textColor),
                            );
                      },
                    ...
                    actions: [
                      TextButton(
                        onPressed: () {
                          ref.read(customSubjectColorsProvider.notifier).remove(subjectId);
          ...
          if (subjectId != null)
            ListTile(
              ...
              title: const Text('Kurs ausblenden', style: TextStyle(color: Colors.red)),
              onTap: () {
                ref.read(filtersProvider.notifier).remove(subjectId);
                Navigator.of(context).pop();
              },
            ),
        ],
